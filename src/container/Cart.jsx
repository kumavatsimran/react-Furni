...
import { clearCart, decrementPro, getNetTotal, incrementPro, removeCart } from '../fetature/product/productSlice';
...
function Cart() {
 ...
 useEffect(() => {
  ...
  <div className="untree_co-section before-footer-section">
    <div className="container">
    <button className="btn btn-outline-black" onClick={()=>dispatch(clearCart(cart))}>clear all</button>
            ...
            <table className="table">
              ...
              <tbody>
                {cart.length>0?(cart.map((product,i)=>{
                  let {id,title,image,price,quantity}=product
                  ...
                  <td>
                    <div className="input-group mb-3 d-flex align-items-center quantity-container" style={{maxWidth: 120}}>
                      <div className="input-group-prepend">
                        <button className="btn btn-outline-black decrease" type="button" onClick={()=>dispatch(decrementPro(i))}>−</button>
                      </div>
                      ...
                      <div className="input-group-append">
                        <button className="btn btn-outline-black increase" type="button" onClick={()=>dispatch(incrementPro(i))}>+</button>
                      </div>
                    </div>
                  </td>
                  <td>${total.toFixed(2)}</td>
                  <td><button type='button' onClick={()=>dispatch(removeCart(id))}>x</button></td>
                </tr>
                  )
                })):(
                  <tr>
